 ...
 */
class CategoriesPartsSearch extends Categories
{
    /**
     * @inheritdoc
     */
    ...
    {
        $query = CategoriesParts::find()->with('parent');///->where(['parent_id'=>0]); // только корневые категории
    ...
    public function searchSubCategories($params)
    {
        ...
        //print_r($params); exit;
        $query = CategoriesParts::find()->where(['parent_id'=>$params['id']]); // только корневые категории
