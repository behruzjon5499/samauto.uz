...
use common\models\CategoriesParts;
use common\models\CategoriesPartsSearch;
use Yii;
 ...
 */
class CategoriesPartsController extends Controller
{
    ...
    {
        $searchModel = new CategoriesPartsSearch();
    ...
    private function update(&$model, $new=false){
        ...
        if( $model->load($post) ) {
            ...
            $model->link_ru = TextHelper::Transliterate( $post['CategoriesParts']['title_ru'] );
            $model->link_uz = TextHelper::Transliterate( $post['CategoriesParts']['title_uz'] );
            $model->link_en = TextHelper::Transliterate( $post['CategoriesParts']['title_en'] );
...
//                if( ! preg_match('/image\//',$file->type) ) return false; // загружена не картинка!
    ...
    {
        $model = CategoriesParts::findOne($id);
        ...
        if( $parent_id == 0 ){
            ...
            if( $cats_sub = CategoriesParts::find()->select(['id','image'])->where(['parent_id'=>$cat_id])->asArray()->all() ){
                $a_cats_sub = [];
            ...
            CategoriesParts::deleteAll(['parent_id'=>$cat_id]);
    ...
    {
        if (($model = CategoriesParts::findOne($id)) !== null) {
            return $model;
        } else {
