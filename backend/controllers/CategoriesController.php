...
class CategoriesController extends Controller
{
    ...
    private function update(&$model, $new=false){
        ...
        if( $model->load($post) ) {
            ...
            $model->link_ru = TextHelper::Transliterate( $post['Categories']['title_ru'] );
            $model->link_uz = TextHelper::Transliterate( $post['Categories']['title_uz'] );
            $model->link_en = TextHelper::Transliterate( $post['Categories']['title_en'] );
...
//                if( ! preg_match('/image\//',$file->type) ) return false; // загружена не картинка!
    ...
    public function actionDelete($id)
    {
        ...
        if( $parent_id == 0 ){
            ...
            if( $cats_sub = Categories::find()->select(['id','image'])->where(['parent_id'=>$cat_id])->asArray()->all() ){
                $a_cats_sub = [];
            ...
            Categories::deleteAll(['parent_id'=>$cat_id]);
